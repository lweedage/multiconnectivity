...
for k in lijstk:
    ...
    if Overload_Failure:
        ...
        if NoReallocation:
            ax2.plot(betas, [fm.outage_probability_overload(k, labdaU / labdaBS, beta) for beta in betas],
                     color=colors[k - 1])
            ax2.scatter(betas, discon, color=colors[k - 1], marker=markers[k - 1], facecolor='none',
                        label=str('$k = $ ' + str(k)))
    ...
    if Distance_Failures:
        ...
        if NoReallocation:
            ...
            ax3.scatter(distances, channel_sum, marker=markers[k - 1], color=colors[k - 1],
                        label=str('$k = $ ' + str(k)), facecolor='none')
        ...
        if NoReallocation:
            ax4.plot(distances, [fm.outage_probability_deterministic_distance(labdaBS, rmax) for rmax in distances],
                     color=colors[k - 1])
            ax4.scatter(distances, discon, color=colors[k - 1], marker=markers[k - 1], facecolor='none',
                        label=str('$k = $ ' + str(k)))
    ...
    if Random_Failure:
        ...
        if NoReallocation:
            ...
            ax8.scatter(probabilities, discon, marker=markers[k - 1], color=colors[k - 1], facecolor='none',
                        label=str('$k = $ ' + str(k)))
    ...
    if LoS_Failure:
        ...
        if NoReallocation:
            ax10.plot(distances, [fm.outage_probability_LoS(labdaBS, k, rlos) for rlos in distances],
                      color=colors[k - 1])
            ax10.scatter(distances, discon, marker=markers[k - 1], color=colors[k - 1], facecolor='none',
                         label=str('$k = $ ' + str(k)))
